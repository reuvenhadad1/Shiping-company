...
public class Van extends Truck{
	...
	public Van(WriteReeLock l) {
		super(l);
		System.out.println("Creating " + this);
	}
	public Van(String licensePlate,String truckModel,WriteReeLock l) {
	...
	public synchronized void deliverPackage(Package p) throws InterruptedException {
		...
		setAvailable(false);
		int time=(p.getDestinationAddress().street%10+1);
			...
			if (!this.isAvailable()) {
				...
				if (this.getTimeLeft()==0){
					for (Package p : this.getPackages()) {
						...
						Tracking tr=new Tracking(MainOffice.getClock(), branch, p.getStatus(),p.getSender_ID());
						p.addTracking(tr);
						try {
					           lock.lockWrite();
					           WriteTracking("Package_ID :"+p.getPackageID()+"-"+tr.toString());
...
//Prototype
	...
	public Van copy() {
		Van VanColne=new Van(this.getLicensePlate(),this.getTruckModel(),lock);
