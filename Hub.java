...
public class Hub extends Branch{
	...
	@Override
	public Node copy() {//source copy restore
		Hub BranchClone=new Hub(2);
		for (int i=0;i<this.listTrucks.size()-1;i++)
			BranchClone.addTruck((StandardTruck) listTrucks.get(i));
		
		BranchClone.addTruck((NonStandardTruck)this.listTrucks.get(this.listTrucks.size()-1));
		for(int i=0;i<this.branches.size();i++)
			((Hub)BranchClone).add_branch((Branch)this.branches.get(i));
