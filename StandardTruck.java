...
public class StandardTruck extends Truck{
	...
	public StandardTruck(String licensePlate,String truckModel,WriteReeLock l,int maxWeight) {
		super(licensePlate,truckModel,l);
		this.maxWeight=maxWeight;
		System.out.println("Creating " + this);
	}
	public StandardTruck(WriteReeLock l) {
		super(l);
	...
	public void unload (Branch dest) {
		...
		synchronized(dest) {
			...
			for (Package p: getPackages()) {
				p.setStatus(status);
				dest.addPackage(p);
				Tracking tr=new Tracking(MainOffice.getClock(), dest, status,p.getSender_ID());
				p.addTracking(tr);	
				try {
			           lock.lockWrite();
	...
	@Override
	public Node copy() {
		StandardTruck TruckColne=new StandardTruck(this.getLicensePlate(),this.getTruckModel(),lock,this.maxWeight);
		return TruckColne;
	}
