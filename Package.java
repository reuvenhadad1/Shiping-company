...
public abstract class Package {
	...
	public Package(Priority priority, Address senderAddress,Address destinationAdress,long Sen) {
		packageID = countID++;
		this.priority=priority;
		this.status=Status.CREATION;
		this.senderAddress=senderAddress;
		this.destinationAddress=destinationAdress;
//		Tracking tr=new Tracking( MainOffice.getClock(), null, status);
//		tracking.add(tr);
		Sender_ID=Sen;
	...
	public void paintComponent(Graphics g, int x, int offset) {
		if (status==Status.CREATION || (branch==null && status == Status.COLLECTION))
   		...
   		g.fillOval(x, 30, 20, 20);
   		
   		if (status==Status.DELIVERED)
   			g.setColor(new Color(204,0,0));
   		else
   			g.setColor(new Color(255,180,180));
   		g.fillOval(x, 578, 20, 20);
