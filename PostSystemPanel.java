...
public PostSystemPanel(Main f) {
   ...
   public void createNewPostSystem(int branches, int trucks, int packages) {
	   if (started) return;
	   game = MainOffice.getInstance(branches, trucks, this, 50);
	   packagesNumber = 50;
	   trucksNumber = trucks;
   ...
   public void PaintBranches(Graphics g,int Bnum) {
	   ...
	   if (game==null) return;
		...
		Hub hub = game.getHub();
	   	...
	   	int offset = 403/(Bnum-1);
	   	int y=100;
	   	int y2=246;
	   	int offset2 = 140/(Bnum-1);
   ...
   public void paintComponent(Graphics g) {
		...
		Hub hub = game.getHub();
	   	...
	   	int x = 50;
	   	int offset3 = (1154-100)/(packagesNumber-1);
...
public void branchInfo() {
   ...
   public void CloneBranch() {
	   ...
	   Jdialogcombo2 j=new Jdialogcombo2(MainOffice.getHub().getBranches().size());
...
//		scrollPane.add(tarea);
	   File file=new File("tracking.txt");
	   JFrame frame = new JFrame();
		frame.setBounds(100, 100, 748, 714);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.getContentPane().setLayout(null);
		
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(0, 0, 734, 677);
   ...
   public void actionPerformed(ActionEvent e) {
	if(e.getSource() == b_num[0]) 
		...
		MainOffice.RestoreBranch();
	    
	 
	    this.branchesNumber=((Hub)MainOffice.getBranchMap().get("0")).getBranches().size();
