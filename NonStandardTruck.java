...
public class NonStandardTruck extends Truck{
	...
	@Override
	public Node copy() {
		NonStandardTruck NonStandardTruckClone=new NonStandardTruck(this.getLicensePlate(),this.getTruckModel(),this.lock, this.length, this. width,  this.height);
		return NonStandardTruckClone;
	}
	
	
	public NonStandardTruck(WriteReeLock l) {
		super(l);
		Random r=new Random();
	...
	public NonStandardTruck(String licensePlate,String truckModel,WriteReeLock l,int length, int width, int height) {
		super(licensePlate,truckModel,l);
		this.width=width;
		this.length=length;
		this.height=height;
	}
			...
			if (!this.isAvailable()) {
				...
				if (this.getTimeLeft()==0) {
					if (p.getStatus()==Status.COLLECTION) {
						...
						Tracking tr=new Tracking(MainOffice.getClock(), null, p.getStatus(),p.getSender_ID());
						p.addTracking(tr);
						try {
					           lock.lockWrite();
